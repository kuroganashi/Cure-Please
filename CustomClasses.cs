...
namespace CurePlease
{
  public class GroupBoxEx : GroupBox
  {
    ...
    {
      var state = base.Enabled ? GroupBoxState.Normal :
          GroupBoxState.Disabled;
      var flags = TextFormatFlags.PreserveGraphicsTranslateTransform |
        ...
        TextFormatFlags flags, GroupBoxState state)
    {
      ...
      rectangle.Width -= 8;
      var size = TextRenderer.MeasureText(g, groupBoxText, font,
  ...
  public class NewTabControl : TabControl
  {
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
      var boundsControl = ClientRectangle;
      ...
      for (var i = 0; i < TabCount; i++)
      {
        ...
        var space = bounds.Height * 1 / 6;
        var imageWidth = 0;
        if (ImageList != null && ImageList.Images.Count >= i + 1)
        {
          imageWidth = bounds.Height * 2 / 3;
          var ins = ImageList.Images[i];
          var rectImage = new Rectangle(new Point(bounds.X + space, bounds.Y + bounds.Height / 2 - imageWidth / 2), new Size(imageWidth, imageWidth));
        ...
        var rectText = new Rectangle(bounds.X + imageWidth + space, bounds.Top, bounds.Width - imageWidth, bounds.Height);
        ...
        var newFont = new Font("Times New Roman", 15);
